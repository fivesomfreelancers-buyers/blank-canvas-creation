...
const BuyerMessages = () => {
  ...
  useEffect(() => { fetchConversations(); }, [location.state]);
  ...
  const fetchConversations = async () => {
    try {
      ...
      const { data: profiles } = await supabase.from('profiles').select('id, full_name, profile_image_url').in('id', partnerIds);
      ...
      const convos: Conversation[] = partnerIds.map(pid => {
        ...
        return {
          ...
          lastMessage: entry.msgs[0].message,
          lastMessageTime: entry.msgs[0].created_at,
      ...
      if (location.state?.openConversation && convos.length > 0) {
        setSelectedChat(convos[0].partnerId);
      }
    } catch (error) {
  ...
  const markAsRead = async (partnerId: string) => {
    if (!currentUserId) return;
    await supabase.from('messages').update({ is_read: true }).eq('sender_id', partnerId).eq('receiver_id', currentUserId).eq('is_read', false);
  };
  ...
  const filteredConvos = conversations.filter(c => c.partnerName.toLowerCase().includes(searchQuery.toLowerCase()));
            ...
            <CardContent className="p-0 flex-1 overflow-y-auto">
              {filteredConvos.length === 0 ? (
                ...
                filteredConvos.map((conv) => {
                  ...
                  return (
                    <div key={conv.partnerId} onClick={() => setSelectedChat(conv.partnerId)} className={`p-4 cursor-pointer hover:bg-accent border-b transition-colors ${selectedChat === conv.partnerId ? 'bg-accent border-l-4 border-l-primary' : ''}`}>
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-center">
                            ...
                            {conv.unreadCount > 0 && <Badge variant="destructive" className="text-xs ml-2">{conv.unreadCount}</Badge>}
                          </div>
                ...
                {selectedConvo ? (
                  <div className="flex items-center space-x-3">
                    <Avatar className="h-8 w-8">
                      ...
                      <AvatarFallback className="bg-primary text-primary-foreground text-xs">{selectedConvo.partnerName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}</AvatarFallback>
                    </Avatar>
