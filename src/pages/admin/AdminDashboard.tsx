...
import {
  ...
  LifeBuoy, Bell, Lock, Activity, Settings as SettingsIcon, Flag, Megaphone, Headphones, Newspaper, Crown, BadgeCheck, Briefcase, Globe,
} from 'lucide-react';
...
import AdminSeoChecks from './AdminSeoChecks';
...
const menuGroups: { label: string; items: { key: string; label: string; icon: any }[] }[] = [
  ...
  { label: 'System', items: [
    ...
    { key: 'seo_checks', label: 'SEO Checks', icon: Globe },
...
const AdminDashboardInner = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'seo_checks': return <AdminSeoChecks />;
  ...
  return (
    <SidebarProvider>
      <div className={`admin-vip fixed inset-0 flex w-full overflow-hidden ${isDark ? 'text-slate-100' : 'text-slate-800'}`}
